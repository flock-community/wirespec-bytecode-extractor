...
Extend `wirespec-extractor` to discover Spring Kafka consumers and producers in a Spring Boot application's compiled classes and emit them as Wirespec `channel` definitions, alongside the existing HTTP `endpoint` extraction. No changes to plugin configuration surface, output directory, or HTTP behavior.
...
All under `extractor-core/src/main/kotlin/community/flock/wirespec/extractor/extract/kafka/` unless noted.
